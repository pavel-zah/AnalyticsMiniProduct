...
Этот репозиторий — лабораторная работа по аналитике данных (Задание 3). Это мини-продукт, где пользователь загружает датасет (CSV), а LLM-агент сам проводит анализ данных через выполнение кода в изолированной среде и возвращает результаты (таблицы, метрики, инсайты, графики).
...
Важно:
- приложение должно использовать LLM через API программно
- LLM должна сама проводить анализ данных через вызов интерпретатора кода (tool / MCP / sandbox), а не только генерировать текст
...
- Графики сохраняются как PNG в sandbox, скачиваются backend-ом и отдаются фронту как ссылки.
